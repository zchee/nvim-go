...
\ {'type': 'command', 'name': 'DlvStdin', 'sync': 1, 'opts': {}},
...
\ {'type': 'command', 'name': 'Gorename', 'sync': 0, 'opts': {'bang': '', 'eval': '[getcwd(), expand(''%:p:h''), expand(''%:p''), line2byte(line(''.''))+(col(''.'')-2), expand(''<cword>'')]', 'nargs': '?'}},
