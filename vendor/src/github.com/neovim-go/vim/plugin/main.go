...
// Main implements the main function for a Neovim remote plugin.
//
// Plugin applications call the Main function to run the plugin. The Main
// function creates a Neovim client, calls the supplied function to register
...
// package for logging. If the environment variable NEOVIM_GO_LOG_FILE is set,
...
func Main(registerHandlers func(p *Plugin) error) {
	...
	v, err := vim.New(os.Stdin, stdout, stdout, log.Printf)
	if err != nil {
		log.Fatal(err)
	}
	p := New(v)
