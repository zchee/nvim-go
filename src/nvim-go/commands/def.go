...
func Def(v *vim.Vim, r [2]int, file string) error {
	...
	if f == nil {
		nvim.Echoerror(v, "Godef: cannot parse %s: %v", filename, err)
	}
	...
	case *ast.ImportSpec:
		...
		if err != nil {
			nvim.Echoerror(v, "Godef: error finding import path for %s: %s", path, err)
		}
		fmt.Println(pkg.Dir)
	case ast.Expr:
		...
		nvim.Echoerror(v, "Godef: no declaration found for %v", pretty{e})
	}
	return nil
}
...
func importPath(v *vim.Vim, n *ast.ImportSpec) string {
	...
	if err != nil {
		nvim.Echoerror(v, "Godef: invalid string literal %q in ast.ImportSpec", n.Path.Value)
	}
	return p
}
...
func findIdentifier(v *vim.Vim, f *ast.File, searchpos int) ast.Node {
	...
	ev := <-ec
	if ev == nil {
		nvim.Echoerror(v, "Godef: no identifier found")
	}
	return ev
}
...
func parseExpr(v *vim.Vim, s *ast.Scope, expr string) ast.Expr {
	...
	if err != nil {
		nvim.Echoerror(v, "Godef: cannot parse expression: %v", err)
	}
	switch n := n.(type) {
	case *ast.Ident, *ast.SelectorExpr:
		return n
	}
	nvim.Echoerror(v, "Godef: no identifier found in expression")
	return nil
}
